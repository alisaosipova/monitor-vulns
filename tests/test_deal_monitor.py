...
def test_parse_cli_item_accepts_plain_name_expression():
    settings = Settings(default_min_roi=0.12, default_min_profit=1.8)
    query = parse_cli_item("AK-47 | Redline (Field-Tested)", settings)
    assert query.market_hash_name == "AK-47 | Redline (Field-Tested)"
    assert query.search_term == "AK-47 | Redline (Field-Tested)"
    assert query.steam_name == "AK-47 | Redline (Field-Tested)"
    assert query.dynamic is False
    assert query.min_roi == 0.12
